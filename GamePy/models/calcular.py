...
class Calcular:
    ...
    def __init__(self: object, dificuldade: int, /) -> None:
        # dificuldade é posicional. Não podemos declarar no objeto
    ...
    def _gerar_resultado(self: object) -> int:
        if self.operacao == 1:    # operação de adição (somar)
            return self.valor1 + self.valor2
        elif self.operacao == 2:  # operação de subtração (diminuir)
            return self.valor1 - self.valor2
        else:                     # operação de multiplicação (multiplicar)
            return self.valor1 * self.valor2
    ...
    def checar_resultado(self: object, resposta: int) -> bool:
        ...
        if self.resultado == resposta:
            ...
            print('\nResposta incorreta!')
        print(
            f'Resposta correta da operação: {self.valor1} {self._op_simbolo} '
            f'{self.valor2} = {self.resultado}\n'
        )
        return certo
